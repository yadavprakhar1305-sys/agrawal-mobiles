...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const result = await db.run('INSERT INTO orders (order_no, user_id, items, total, address, city, pincode, phone, payment_method, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    for (const item of items) {
      await db.run('UPDATE products SET stock = stock - 1 WHERE id = ? AND stock > 0', [item.product_id || item.id]);
    }
...
router.get('/', auth, async (req, res) => {
  try {
    const orders = await db.queryAll('SELECT * FROM orders WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
