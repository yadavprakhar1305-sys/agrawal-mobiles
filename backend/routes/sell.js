...
router.post('/', auth, async (req, res) => {
  try {
    ...
    await db.run('INSERT INTO sell_requests (user_id, brand, model, condition, asking_price, description, images) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
router.get('/', auth, async (req, res) => {
  try {
    const requests = await db.queryAll('SELECT * FROM sell_requests WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
    res.json(requests);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
